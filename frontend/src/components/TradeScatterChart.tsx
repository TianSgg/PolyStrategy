...
function buildOption(trades: Trade[], darkMode: boolean, midPrice?: number | null): EChartsOption {
  ...
  const leaderData: any[] = sorted.map((t, i) => ({
  ...
  const lPositionData = lPositionRaw
  ...
  if (midPrice != null) {
    ...
    lPositionData.push(Math.max(0, lPosAccum))
    leaderData.push({
      value: [categories.length - 1, midPrice],
...
export default function TradeScatterChart({ trades, darkMode, midPrice }: TradeScatterChartProps) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'grid', gridTemplateColumns: cols, justifyContent: 'end', padding: '2px 8px', fontSize: 11, fontWeight: 600, flexShrink: 0, gap: '1px 12px', fontVariantNumeric: 'tabular-nums' }}>
        ...
        <span style={{ color: followerColor }}>F</span>

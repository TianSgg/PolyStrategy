...
列表中的红色状态只用于需要关注的执行异常：接口/程序错误、订单或成交校准异常、未清仓风险。`force_exit` 和 `buy_placement_failed` 这类流程性终止默认不使用红色；资金不足、未下单、买入无成交、买入部分成交后正常结束、入场超时和止损退出也不使用红色。这些状态分别使用灰色、琥珀色或绿色表达结果，不制造失败告警。部分成交本身按绿色处理，只有系统异常才升级为红色。

...
def get_copy_trading_configs(enabled_only: bool = False) -> List[CopyTradingConfig]:
    ...
    try:
        if enabled_only:
            cursor.execute("""
                SELECT id, leader_proxy_wallet, follower_proxy_wallet, threshold, allowance,
                       ...
                       buy_follow_taker, sell_follow_taker, auto_merge_enabled, auto_merge_threshold, buy_only,
        ...
        return [CopyTradingConfig(
            ...
            auto_merge_enabled=bool(row[19]) if row[19] is not None else False,
            auto_merge_threshold=float(row[20]) if row[20] is not None else 100.0,
            buy_only=bool(row[21]) if row[21] is not None else False,
            buy_price_filter_min=float(row[22]) if row[22] is not None else DEFAULT_BUY_PRICE_FILTER_MIN,
            buy_price_filter_max=float(row[23]) if row[23] is not None else DEFAULT_BUY_PRICE_FILTER_MAX,

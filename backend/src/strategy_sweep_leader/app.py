...
async def lifespan(app: FastAPI):
    ...
    runtime = SingleStrategyRuntime(
        strategy_type=StrategyType.SWEEP_LEADER,
        strategy_class=SweepLeaderStrategy,
    )
    await runtime.start()
    app.state.runtime = runtime

    logger.info("Strategy SweepLeader service started on port %d", PORT)
...
app = FastAPI(title="Strategy Service (sweep_leader)", lifespan=lifespan)
